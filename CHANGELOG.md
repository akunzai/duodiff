...
- Reworked the visual language across the TUI and the docs so the two panes stay column-aligned and read as structure rather than decoration. Directory rows now carry a single-width `▸` / `▾` disclosure marker and a trailing `/` in place of the `📁` / `📄` emoji, and the indicator column uses `<` / `>` / `!` in place of `⬅` / `➡` / `💥`. Help and `docs/SHORTCUTS.md` also had the left/right sense of the one-sided marker corrected — it names the side the entry is on, which the old wording had inverted. Command Palette labels moved to sentence case with verbs consistent with Help (a change block is *staged*, then *saved*), and the settings screen is now called **Config** everywhere instead of alternating with *Configuration*.
- The README is now a short value proposition, one install path, one quick start, and the demo; the feature tour moved to `docs/FEATURES.md` and the settings table to `docs/CONFIGURATION.md`. `docs/design.md` records the principles behind all of this.
...
- External diff tool support now provides explicit **Auto** (default; resolves the first launchable tool by fixed priority: `vim`, `nvim`, `code`, `meld`, `bcomp`, `smerge`, `ksdiff`, `difft`), **Disabled**, and pinned tool settings. Unavailable tools remain visible as `[-] tool (Not Found)` in dim text and are skipped during keyboard navigation; mouse clicks and Enter/Space are no-ops on them. Pinned tools that are missing display as `[x] tool (Not Found)` without falling back or mutating settings, and unknown values from configuration files display as warning rows and are preserved on save. Tool availability is refreshed upon opening Config and re-validated immediately before external diff handoff; missing or disabled tools stay in the TUI with an error toast instead of failing after dropping alternate screen. The Config screen top border now features responsive contextual title hints (`select`, `toggle`, `adjust`, `Not Found`) that drop as whole units on narrow terminals while reserving space for the close button (Issue #249).
- Filtering the directory tree now searches the entire scanned hierarchy regardless of directory collapse state, displaying matches as a flat list with context-preserving breadcrumbs (`parent/path › name`) that abbreviate in the middle when narrow. When the filter bar is open, footer hints display responsive instructions (`Enter:apply · Esc:cancel · Ctrl+f:diffs only`) dropped strictly by complete unit on narrow terminals. Directory-tree alignment now matches entries differing only in case across sides into paired rows using Unicode case folding and normalization, retaining side-specific relative paths and basenames for diffing, editing, and copying. Casing mismatches display an `Aa` badge in the indicator column and separate left/right names in the footer metadata strip, while ambiguous case collisions (>1 match candidate) remain separate single-sided rows and block copy actions with an informative toast (Issue #248).
