        ...
        for op in group {
            for change in diff.iter_changes(&op) {
                ...
                match change.tag() {
                    ChangeTag::Equal => {
                        rows.push((
                            Some(DiffLine { tag: ChangeTag::Equal, text: line_content.clone() }),
                            Some(DiffLine { tag: ChangeTag::Equal, text: line_content }),
                        ));
                    }
                    ChangeTag::Delete => {
                        rows.push((
                            Some(DiffLine { tag: ChangeTag::Delete, text: line_content }),
                            None,
                        ));
                    }
                    ChangeTag::Insert => {
                        rows.push((
                            None,
                            Some(DiffLine { tag: ChangeTag::Insert, text: line_content }),
                        ));
                    }
                }
            }
...
mod tests {
    ...
    fn test_compare_files_basic() {
        ...
        let has_delete = rows.iter().any(|(left, right)| {
            left.as_ref().is_some_and(|l| l.tag == ChangeTag::Delete && l.text.contains("world")) && right.is_none()
        });
        let has_insert = rows.iter().any(|(left, right)| {
            left.is_none() && right.as_ref().is_some_and(|r| r.tag == ChangeTag::Insert && r.text.contains("bar"))
        });
