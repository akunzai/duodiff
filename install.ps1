<#
.SYNOPSIS
  duodiff installer for Windows — downloads the prebuilt release binary, verifies
...
.DESCRIPTION
  ...
  To pass options, invoke it as a script block:
    & ([scriptblock]::Create((irm https://raw.githubusercontent.com/akunzai/duodiff/main/install.ps1))) -Version v0.2.0

.PARAMETER Version
  Install a specific release tag (default: latest). Env: DUODIFF_VERSION.

.PARAMETER BinDir
  Install directory (default: ~\.local\bin). Env: DUODIFF_BIN_DIR.
...
[CmdletBinding()]
param(
  [string]$Version = $(if ($env:DUODIFF_VERSION) { $env:DUODIFF_VERSION } else { 'latest' }),
  [string]$BinDir  = $(if ($env:DUODIFF_BIN_DIR) { $env:DUODIFF_BIN_DIR } else { '' }),
  [switch]$Help
)
...
$Repo = 'akunzai/duodiff'

function Die([string]$msg) { throw "error: $msg" }

if ($Help) {
  @'
duodiff installer (Windows / PowerShell)

  irm https://raw.githubusercontent.com/akunzai/duodiff/main/install.ps1 | iex

Options (via env vars, or as a script block):
  -Version <tag>   install a specific release (default: latest)   [DUODIFF_VERSION]
  -BinDir  <dir>   install directory (default: ~\.local\bin)       [DUODIFF_BIN_DIR]

  & ([scriptblock]::Create((irm <url>))) -Version v0.2.0 -BinDir C:\tools\bin
'@ | Write-Host
  return
}
...
if ($Version -eq 'latest') {
  try {
    $rel = Invoke-RestMethod -UseBasicParsing -Headers @{ 'User-Agent' = 'duodiff-installer' } `
...
$pkg     = "duodiff-$Version-$target"
$archive = "$pkg.zip"
$baseUrl = "https://github.com/$Repo/releases/download/$Version"

# --- Download + verify --------------------------------------------------------
$tmp = New-Item -ItemType Directory -Path (Join-Path ([IO.Path]::GetTempPath()) "duodiff-$([Guid]::NewGuid())")
try {
  ...
  $src = Join-Path $tmp "$pkg\duodiff.exe"
  if (-not (Test-Path $src)) { Die "binary not found in archive at $pkg\duodiff.exe" }

  # --- Install ----------------------------------------------------------------
  New-Item -ItemType Directory -Force -Path $BinDir | Out-Null
  $dest = Join-Path $BinDir 'duodiff.exe'
  Copy-Item -Path $src -Destination $dest -Force
  Write-Host "installed duodiff $Version to $dest"
} finally {
...
# Make duodiff usable in this session too.
