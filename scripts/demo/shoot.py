#!/usr/bin/env python3
"""Drive the real duodiff binary to a single screen and write an asciinema v2
.cast whose *final frame* is that screen — the still source for a PNG
screenshot.

This mirrors record.py's pty driver, with one deliberate difference: the cast
is written *before* the duodiff process is torn down, so the terminal
teardown (raw mode off / leave alternate screen) never lands in the capture
and blanks the last frame. The storyboard is the same ["wait", secs] /
["key", name-or-literal] format as record.py; all paths come from the
environment (see shots.sh for the contract).
"""
import codecs
import fcntl
import json
...
# Keep these in sync with record.py — the same key names and recording theme
# so a shot frames identically to the demo GIF.
KEYS = {
    "enter": "\r",
    "esc": "\x1b",
    "tab": "\t",
...
def main():
    ...
    # Write the cast NOW, before tearing the process down: duodiff's teardown
    # leaves the alternate screen (a clear), which must not become the
    # captured last frame.
    header = {
        "version": 2,
        "width": COLS,
